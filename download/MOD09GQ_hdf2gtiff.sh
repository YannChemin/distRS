...
#for year in 2000 2001 2002 2003 2004 2005 2006 #2007 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019
...
do
	...
	do
		echo $year$doy
		file=$(ls MOD09GQ.A$year$doy.h19v04*.hdf)
		...
		gdalwarp -of GTiff -q -cutline $cutlineV -crop_to_cutline -dstnodata $NODATA -s_srs '+proj=sinu +R=6371007.181 +nadgrids=@null +wktext' -t_srs $LAEA -te $xmin $ymin $xmax $ymax "${SDS11#*=}" $outRoot/$out.tif &
		file=$(ls MOD09GQ.A$year$doy.h19v04*.hdf)
		SDS11=$(gdalinfo $file | grep "SUBDATASET_4_NAME")
		out=$(echo ${SDS11#*=} | sed 's/[\/\"\:\.\ ]/\_/g' | sed 's/\(.*\)_h..v.._\(.*\)/\1\_\2/g')
		echo "OUT=" $out
		...
		rm -f $outRoot/$out.tif
		gdalwarp -of GTiff -q -cutline $cutlineV -crop_to_cutline -dstnodata $NODATAQC -s_srs '+proj=sinu +R=6371007.181 +nadgrids=@null +wktext' -t_srs $LAEA -te $xmin $ymin $xmax $ymax "${SDS11#*=}" $outRoot/$out.tif 
	done
done
