...
// 	printf("Passed 4\n");
	...
	float *matdtdry = (float *) malloc(sizeof(float)*N);
	float *matdtdryidx = (float *) malloc(sizeof(float)*N);
	float *matalbidx = (float *) malloc(sizeof(float)*N);
	float *matevapfrsseb = (float *) malloc(sizeof(float)*N);
	
	int rowcol;
	float tempk, etpotd;
	float Rn, g0;
	float ndvi_max=0.0;
	float albedo_max=0.0001, albedo_min=0.9999;//init values
	double dt;//dtair map out
	float t0dem_min=400.0, t0dem_max=0.0;
	float dtdry_min=2000, dtdry_max=0.0;
	float index, index_min=2000, index_max=0.0;
	float h0, dem, t0dem ;
	float Rn_dry, g0_dry;
	float Rn_wet, g0_wet;
...
// 	printf("Passed 5\n");
	...
	#pragma omp parallel for default(none) \
		private(rowcol, tempk, etpotd, Rn, g0, dem, t0dem,)\
		shared(N, nX,  nY, roh_w, tsw, doy, u2m,\
			t0dem_min,t0dem_max,dtdry_min,dtdry_max,\
	...
	for(rowcol=0;rowcol<N;rowcol++){
		if(mat2[rowcol]==-28768||mat8[rowcol]==-28768||mat2[rowcol]<=0.001){
			...
			if (t0dem>t0dem_max&&t0dem>274) t0dem_max = t0dem;
			if (t0dem<t0dem_min&&t0dem>274) t0dem_min = t0dem;
			etpotd = et_pot_day( mat15[rowcol], tempk, roh_w );
			matOut3[rowcol] = etpotd;
			Rn = mat16[rowcol];
			g0 = mat17[rowcol];
			matdtdry[rowcol]=0.2*(Rn-g0)/u2m;
			if(matdtdry[rowcol]>100) matdtdry[rowcol]=0;
			if (matdtdry[rowcol]>dtdry_max) dtdry_max = matdtdry[rowcol];
			if (matdtdry[rowcol]<dtdry_min) dtdry_min = matdtdry[rowcol];
		}
	}
	#pragma omp barrier
	printf("Albedo_min\t= %7.5f\tAlbedo_max\t= %7.5f\n",albedo_min,albedo_max);
	printf("t0dem_min\t= %7.2f\tt0dem_max\t= %7.2f\n\n",t0dem_min,t0dem_max);
...
// 	printf(" (if it has rogue values to clean)\n");
	...
	#pragma omp parallel for default(none) \
		private(rowcol, tempk, etpotd, Rn, g0, dem, t0dem,index)\
		shared(N,index_min, index_max,\
			t0dem_min, t0dem_max, dtdry_max, dtdry_min,\
			albedo_min,albedo_max, i_bottom1a, i_peak3,\
			Rn_dry, g0_dry, Rn_wet, g0_wet, t0dem_wet, t0dem_dry,dem_dry, \
			mat2,mat8,mat14,mat15,mat16,mat17,\
			matdtdry, matdtdryidx, matalbidx, matevapfrsseb, \
			matOut3 )
	for(rowcol=0;rowcol<N;rowcol++){
		tempk = mat8[rowcol] * 0.02;
		dem = mat14[rowcol];
		t0dem = tempk + 0.00627 * dem;
		if(t0dem_min<274) t0dem_min=274;
		if(t0dem_min<i_bottom1a&&i_bottom1a<t0dem_max) t0dem_min= i_bottom1a;
		if(t0dem_max>350||t0dem_max>i_peak3) t0dem_max= i_peak3; 
		if(mat2[rowcol]==-28768||mat8[rowcol]==-28768||mat2[rowcol]<=0.001||
		  t0dem<274||mat16[rowcol]==-28768||mat17[rowcol]==-28768){
			matdtdryidx[rowcol]=-28768;
			matalbidx[rowcol]=-28768;
			matevapfrsseb[rowcol]=-28768;
		} else {
			matdtdryidx[rowcol]=idx(dtdry_max,dtdry_min,matdtdry[rowcol]);
			matalbidx[rowcol]=idx(albedo_max, albedo_min,mat2[rowcol]*0.001);
			matevapfrsseb[rowcol]=idx(t0dem_max,t0dem_min,t0dem);
			Rn = mat16[rowcol];
			g0 = mat17[rowcol];
			index=matdtdryidx[rowcol]*(1-matevapfrsseb[rowcol])*matalbidx[rowcol];
			if(index<index_min){
				index_min=index;
				t0dem_wet=t0dem;
				Rn_wet=Rn;
	...
	#pragma omp barrier
	printf("t0dem_min\t= %7.2f\tt0dem_max\t= %7.2f\n\n",t0dem_min,t0dem_max);
	printf("Rn_wet\t\t= %7.2f\t",Rn_wet);
	printf("Rn_dry\t\t= %7.2f\n",Rn_dry);
	printf("g0_wet\t\t= %7.2f\t",g0_wet);
	printf("g0_dry\t\t= %7.2f\n",g0_dry);
	printf("LE_wet\t\t= %7.2f\t",Rn_wet-g0_wet);
	printf("H_dry\t\t= %7.2f\n",Rn_dry-g0_dry);
	printf("LE_wet\t\t= %5.2f\t\t",(Rn_wet-g0_wet)*100/Rn_wet);
	printf("H_dry\t\t= %5.2f\n\n",(Rn_dry-g0_dry)*100/Rn_dry);
	printf("\t\t\t\tdem_dry\t\t= %7.2f\n",dem_dry);
	float score=0.0;
	int score_max=1;
	...
	#pragma omp parallel for default(none) \
		...
		shared (N,iteration,ndvi_max,Rn_dry,g0_dry,score,score_max, \
			 t0dem_wet,t0dem_dry,u2m,doy,dem_dry, \
			mat1,mat2,mat8,mat14,mat15,mat16,mat17, \
