#!/bin/bash
Root=~/ladsweb.nascom.nasa.gov/allData/5/MOD13A2
xmin=79.4
ymin=5.9
xmax=82.0
ymax=9.9
...
do
	...
	do
		...
		echo $location "/" $day
		rm -f *.xml
		if [ $( find . -name "MOD13A2*h25v08*.hdf" | wc -l ) -eq 1 ]
		then
			file=$(ls MOD13A2*h25v08*.hdf)
			SDS0=$(gdalinfo $file | grep "SUBDATASET_1_NAME")
			out=$(echo ${SDS0#*=} | sed 's/[\/\"\:\.\ ]/\_/g' | sed 's/\(.*\)_h..v.._\(.*\)/\1\_\2/g')
			file=$(ls MOD13A2*h26v08*.hdf)
...
do
	...
	do
		...
		then
			file=$(ls MOD13A2*h25v08*.hdf)
			SDS0=$(gdalinfo $file | grep "SUBDATASET_12_NAME")
			file=$(ls MOD13A2*h26v08*.hdf)
			SDS1=$(gdalinfo $file | grep "SUBDATASET_12_NAME")
			out=$(echo ${SDS0#*=} | sed 's/[\/\"\:\.\ ]/\_/g' | sed 's/\(.*\)_h..v.._\(.*\)/\1\_\2/g')
			echo "OUT=" $out
			#Define number of gdalwarp running
			npid=$(echo "$(ps aux | grep gdalwarp | wc -l) - 1" | bc)
