...
echo $xpix $ypix
height=$(gdalinfo $(echo $file | sed 's/.tar.gz//g') | grep 'Size is' | sed 's/Size\ is\ \(.*\),\(.*\)/\2/g')
width=$(gdalinfo $(echo $file | sed 's/.tar.gz//g') | grep 'Size is' | sed 's/Size\ is\ \(.*\),\(.*\)/\1/g')
...
gdalwarp -of GTiff -ot Float32 -t_srs $WGS84 -te $xmin $ymin $xmax $ymax -ts $width $height ~/Osaka/SRTM/srtm_tonlesap.tif srtm.tif
